...
def summary(model):
    ...
    result = []
    for layer in model.layers:
        # Get shape from output tensor for all layers
        ...
        # Set batch dimension to None
        shape_list[0] = None
        
        # Handle InputLayer format differently (needs to be wrapped in a list)
        if layer.__class__.__name__ == 'InputLayer':
            shape = [tuple(shape_list)]
        else:
            shape = tuple(shape_list)
        
        descriptors = [layer.__class__.__name__, shape, layer.count_params()]
